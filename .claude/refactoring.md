...
**When refactoring code, delegate to the `test-first-partner` agent.**

Refactoring lives inside the red-green-refactor cycle. The agent enforces the discipline of only refactoring on green, making one small change at a time, and running tests after each step.
...
4. **If red, undo** - Take a smaller step
5. **Repeat** - Small improvements, committed frequently
